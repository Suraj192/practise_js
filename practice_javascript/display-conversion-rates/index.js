...
const API = new FetchWrapper(
  "https://v6.exchangerate-api.com/v6/633dc66c36937369619e0b29/latest/"
);
const exchangeRates = () => {
  ...
  API.get(base).then((data) => {
    Object.keys(data.conversion_rates).forEach((key) => {
      if (key == target) {
        result.textContent = data.conversion_rates[key];
      }
    });
  });
};

baseCurrency.addEventListener("change", exchangeRates);
targetCurrency.addEventListener("change", exchangeRates);
