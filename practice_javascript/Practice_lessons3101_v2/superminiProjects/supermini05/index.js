...
function addGrade(grades, grade) {
  grades.splice(grades.length, 0, Number(grade));
  return grades;
}
...
function getFirstGrade(grades) {
  return `First submited grade is ${grades[0]}`;
}
...
function getLastGrade(grades) {
  return `Last submitted length is ${grades[grades.length - 1]}`;
}
...
function exportCSV(grades) {
  const grade_arr = grades.map((grade) => String(grade));
